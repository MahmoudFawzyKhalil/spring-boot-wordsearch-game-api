...
},{"./grid":"5VEEk"}],"5VEEk":[function(require,module,exports) {
...
class Grid {
    ...
    getCellsInRange(firstLetter, currentLetter) {
        ...
        if (firstLetter.y === currentLetter.y) {
            ...
            for(let i = firstLetter.x; i <= currentLetter.x; i++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${i}"][data-y="${currentLetter.y}"]`));
        // Vertical
        } else if (firstLetter.x === currentLetter.x) {
            if (firstLetter.y === currentLetter.y) return cellsInRange;
            if (firstLetter.y > currentLetter.y) [currentLetter, firstLetter] = [
                firstLetter,
                currentLetter
            ];
            for(let i = firstLetter.y; i <= currentLetter.y; i++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${currentLetter.x}"][data-y="${i}"]`));
        // Diagonal: both x and y increase or decrease together
            ...
            for(let i = 0; i <= currentLetter.y - firstLetter.y; i++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x + i}"][data-y="${firstLetter.y + i}"]`));
        // Inverse diagonal: x increases and y decreases or the opposite
            ...
            for(let i = 0; i <= currentLetter.x - firstLetter.x; i++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x + i}"][data-y="${firstLetter.y - i}"]`));
        }
        return cellsInRange;
    }
    // Event Handlers
    ...
    moveHandler(e) {
        ...
        const x = +cell.dataset.x;
        const y = +cell.dataset.y;
        this.selectedItems.forEach((cell1)=>cell1.classList.remove("selected")
        );
        ...
        this.selectedItems.forEach((cell1)=>cell1.classList.add("selected")
        );
    ...
    upHandler(e) {
        ...
        if (this.words.indexOf(selectedWord) !== -1) {
            ...
            this.selectedItems.forEach((cell)=>cell.classList.add("selected-correct")
            );
            this.selectedItems = [];
            ...
            this.selectedItems = [];
            this.success.classList.remove("hidden");
        } else this.selectedItems.forEach((cell)=>cell.classList.remove("selected")
        );
...
},{}]},["hTbRF","c1VqH"], "c1VqH", "parcelRequire938b")
